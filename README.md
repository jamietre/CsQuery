...
CsQuery uses a new HTML parser that is oriented towards HTML5 compliance. As of development release after 1.1.1.x, it respects HTML5 rules for optional tags. It will now generate optional (but structurally required) tags such as `<tbody> .. </tbody>` and optional closing tags such as `</p>`. This means you should be able to expect any selector to return exactly the same results in CsQuery as it does in a web browser. Though far from complete, at this point it should compare favorably to other existing HTML parsers in .NET in terms of producing HTML that is faithful to the HTML a browser would render. I plan to implement other parts of the specification, especially the rules for parsing invalid HTML, in the future.
